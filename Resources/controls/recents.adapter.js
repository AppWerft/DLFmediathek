...
Module.prototype = {
	...
	setProgress : function(_progress) {
		var link = Ti.Database.open(DB);
		link.execute('UPDATE recents SET progress=?,lastaccess=? WHERE url=?', _progress, Moment().toISOString(), this.url);
		link.close();
	},
	...
	getAllRecents : function() {
		var link = Ti.Database.open(DB);
		var recents = [];
		var res = link.execute('SELECT * FROM recents ORDER BY DATETIME(lastaccess) DESC');
		while (res.isValidRow()) {
			...
			recents.push({
				...
				color : (station )? Model[res.getFieldByName('station')].color :'gray'
			});
			res.next();
		}
	...
	getProgress : function() {
		var link = Ti.Database.open(DB);
		var res = link.execute('SELECT progress FROM recents  WHERE url=?', this.url);
		var progress = 0;
		if (res.isValidRow()) {
			progress = res.getFieldByName('progress');
			res.close();
