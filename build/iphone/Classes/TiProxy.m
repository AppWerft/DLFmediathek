...
//Rare exceptions to indicate a bug in the _dlrmediathek code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_dlrmediathek = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_dlrmediathek]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.dlrmediathek.module"
